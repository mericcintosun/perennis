...
import { RollLoopMark } from "@/components/brand/marks";
...
export default function Home() {
  return (
    <div>
      {/* The masthead. No image inside the text column, no card, no badge over
          the headline. The one ornament is the window grid in the right margin
          from lg up, at 40 percent, hidden from assistive technology, and it is
          a bordered drawing rather than a wash: there is no mask on this page. */}
      <section className="border-b border-border">
        <div className="relative mx-auto max-w-6xl px-6 py-16 sm:py-24">
          <Image
            ...
            height={120}
            priority
            className="pointer-events-none absolute right-6 top-20 hidden w-52 opacity-40 lg:block"
          />

          <div className="pns-masthead relative max-w-[68ch]">
            <h1 className="pns-ink-in font-serif text-5xl leading-[1.02] sm:text-7xl">
              Hold a view for four hours. Sign once.
            </h1>

            <p className="pns-ink-in mt-8 max-w-[68ch] text-lg leading-relaxed text-muted-foreground">
              ...
              every settlement with your stop rules written in as contract terms.
            </p>
            ...
            <div className="pns-ink-in mt-12">
              <span aria-hidden className="pns-rule block h-px w-full bg-border" />
      ...
      {/* How it works. Same three beats as DEMO.md steps 1, 4 and 6, in the
          dispatch form, static, no state and no tabs. */}
      <section id="how" className="scroll-mt-20 border-b border-border">
        <div className="mx-auto max-w-6xl px-6 py-16 sm:py-20">
          <SectionRule label="How it works" />
          <div className="mt-8 flex items-start justify-between gap-8">
            <h2 className="max-w-[68ch] font-serif text-3xl leading-tight sm:text-4xl">
              Three steps, and only the first one is yours
            </h2>
            {/* The roll loop mark: one window closes, the next opens, and the
                loop closes itself. Decorative, and it takes its colour from the
                text around it rather than carrying one. */}
            <RollLoopMark className="hidden size-12 shrink-0 text-muted-foreground sm:block" />
          </div>
          <p className="mt-4 max-w-[68ch] text-[15px] leading-relaxed text-muted-foreground">
            The vault is a contract you deploy and own, and it holds the plan,
            the collateral and the queue.
          </p>
          <DispatchList entries={howItWorks} className="mt-12" />
        </div>
      </section>

      {/* What is running today. Three numbers, inside sentences, then the long
          form comparison folded away underneath them. */}
      <section className="border-b border-border">
        <div className="mx-auto max-w-6xl px-6 py-16 sm:py-20">
